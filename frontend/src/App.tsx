  ...
  const adicionarCliente = async (e: React.FormEvent) => {
    ...
    try {
      if (clienteEditandoId) {
        await axios.put(`${API_URL}/${clienteEditandoId}`, {
          nome,
          telefone,
          email,
        });
      ...
      if (error.response && error.response.data && error.response.data.erro) {
        ...
        setMensagemErro(
          "Erro ao conectar com o backend. O servidor está rodando?",
        );
      }
    }
>>>>>>> Stashed changes
...
=======
    try {
      ...
      alert(
        "Erro ao gerar relatório. Rota não encontrada ou servidor offline.",
      );
    }
  };
  ...
  const salvarAgendamento = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (new Date(dataHoraCombinada) < new Date()) {
        setMensagemErro(
          "O horário selecionado já passou. Escolha um horário no futuro.",
        );
        return;
      }
...
=======
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
      <button
        ...
        className="bg-white p-8 rounded-3xl shadow-sm border-2 border-transparent hover:border-indigo-500 transition-all flex flex-col items-center gap-4 group"
      >
>>>>>>> Stashed changes
...
=======
      ...
      <button
        ...
        className="bg-white p-8 rounded-3xl shadow-sm border-2 border-transparent hover:border-amber-500 transition-all flex flex-col items-center gap-4 group"
      >
>>>>>>> Stashed changes
        ...
        {tela === "novo" && (
          <div className="bg-white p-8 rounded-3xl shadow-sm max-w-lg mx-auto border border-slate-100">
            <h2 className="text-2xl font-bold mb-6 flex items-center gap-2 text-indigo-600">
              <UserPlus />{" "}
              {clienteEditandoId ? "Alterar Cliente" : "Cadastrar Cliente"}
            </h2>
...
=======
              ...
              <input
                className="border-2 border-slate-100 p-3 rounded-xl outline-none focus:border-indigo-500 transition"
                placeholder="Nome"
                value={nome}
              ...
              />
              <button className="bg-indigo-600 text-white font-bold py-4 rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition">
                {clienteEditandoId ? "Salvar Alterações" : "Salvar Cliente"}
              </button>
>>>>>>> Stashed changes
            </form>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex gap-2">
              <div className="relative flex-1">
                ...
                />
                <input
                  className="w-full bg-slate-50 p-2.5 pl-10 rounded-xl outline-none"
            ...
            <div className="grid gap-4">
              {clientes.map((c) => (
                <div
                  key={c.id}
                  className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex justify-between items-center hover:border-indigo-200 transition"
                >
                  <div>
                    <p className="font-bold text-lg">{c.nome}</p>
                    <p className="text-slate-500 text-sm">
                      {c.telefone} | {c.email || "Sem e-mail"}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => exibirCliente(c.id)}
                    ...
                    <button
                      ...
                      className="text-slate-400 hover:text-amber-600 p-2 transition"
          ...
          <div className="space-y-6 max-w-2xl mx-auto">
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
              ...
              >
                ...
                <select
                  className="border-2 border-slate-100 p-3 rounded-xl outline-none focus:border-rose-500 transition"
                  value={clienteAgendamento}
                  onChange={(e) => setClienteAgendamento(e.target.value)}
                  required
                >
                ...
                <select
                  className="border-2 border-slate-100 p-3 rounded-xl outline-none focus:border-rose-500 transition"
                  value={servicoAgendamento}
                  onChange={(e) => setServicoAgendamento(e.target.value)}
                  required
                >
                ...
                <div className="grid grid-cols-2 gap-4">
                  <input
                    ...
                    className="border-2 border-slate-100 p-3 rounded-xl outline-none focus:border-rose-500 transition w-full"
                ...
                <button className="bg-rose-600 text-white font-bold py-4 rounded-xl hover:bg-rose-700 shadow-lg shadow-rose-100 transition">
            ...
            <div className="grid gap-4">
              {servicos.map((s) => (
                ...
                >
                  ...
                  <p className="text-emerald-600 font-bold bg-emerald-50 px-4 py-1 rounded-full">
                    R$ {s.preco}
                  </p>
                </div>
              ))}
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
              <div className="text-center mb-10">
                ...
                <p className="text-slate-500">
                  Gerado em:{" "}
                  {Number.isNaN(new Date(relatorioData.dataGeracao).getTime())
                    ? relatorioData.dataGeracao
                    : new Date(relatorioData.dataGeracao).toLocaleString(
                        "pt-BR",
                      )}
                </p>
              </div>
                ...
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-inner">
                  ...
                  <p className="text-4xl font-black text-emerald-600">
                    {new Intl.NumberFormat("pt-BR", {
                      style: "currency",
                      currency: "BRL",
                    }).format(relatorioData.faturamentoTotal || 0)}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-8 border-t border-slate-100 pt-8">
                ...
                {relatorioData.servicosMaisPedidos &&
                relatorioData.servicosMaisPedidos.length > 0 ? (
